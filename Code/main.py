...
max_frequency = 300 #max allowable frequency [Hz]
min_frequency = 12 #min freq [Hz]
...
button_half_freq = machine.Pin(21, machine.Pin.IN, machine.Pin.PULL_UP)
button_double_freq = machine.Pin(14, machine.Pin.IN, machine.Pin.PULL_UP)
button_reset_freq = machine.Pin(2, machine.Pin.IN, machine.Pin.PULL_UP)
pin_sig = machine.PWM(machine.Pin(20))
...
while True:
    ...
    if freq_change == True:
        ...
        oled.fill(0)
        Writer.set_textpos(oled, 0, 0)
        wrt.printstring(str(frequency/4))
